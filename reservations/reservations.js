...
const reserveRoom = (req, res) => {
    if (!fs.existsSync('./roomList/reservations.json')) {
        ...
        if (!reservationsList.some(reservation => !checkSchedule(renameKeys({reservationStartTime: 'queryStartTime', reservationEndTime: 'queryEndTime'}, req.body), reservation))){
...
const filterRooms = (roomsList, query) => {
    if (!fs.existsSync('./roomList/reservations.json')){
        ...
        return roomsList.filter(room => !JSON.parse(reservationsList).some(reservation => (checkName(reservation.name, room.name) && !checkSchedule(query, reservation)) || !checkCapacityAndEquipements(query, room)));
    }
};
...
const checkSchedule = ({queryStartTime ,queryEndTime}, {reservationStartTime, reservationEndTime}) => {
    console.log(queryStartTime, queryEndTime, reservationStartTime, reservationEndTime);
    console.log("CHECKSCHEDULE", (queryStartTime < reservationStartTime && queryEndTime < reservationStartTime) || (queryStartTime > reservationEndTime && queryEndTime > queryStartTime));
    return (queryStartTime < reservationStartTime && queryEndTime < reservationStartTime) || (queryStartTime > reservationEndTime && queryEndTime > queryStartTime);
};
